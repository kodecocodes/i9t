...
Do whichever of the above three options you like to run your unit tests. Each tells Xcode to build and launch your app, and then run your tests. In the **test navigator**, you should see six total tests:
...
Name the class **DataModelTests**, and ensure it's a subclass of **XCTest** and that **Swift** is selected. Click **Next** and then **Create**.
...
Both options have downsides. With option one, you know that adding files to both targets is a manual step that's easily forgotten. It also doesn't make much sense conceptually. The testing module should only contain tests!
...
``` 
One of the ways the UI testing framework references individual UI components in your app is via their accessibility information. Setting the accessibility identifier for `UITableView` to `"Workouts Table"` will let you use the string as a reference to the table later on.
...
You may be thinking that this is not a real test since it has no assertions. If you've written regular XCTest tests before, you know that they rely on one or more assertions, such as `XCTAssertTrue`, `XCTAssertFalse` or `XCTAssertEquals`.

Although you _can_ add assertions, you don't have to explicitly assert anything in a UI test. If the test expects to find a specific UI element on the screen, like a button that says _Workouts_, but doesn’t find it, the test will fail. In other words, tapping through your app _implicitly_ tests your UI.

### UI test classes

Quickly take a deeper look at some of the details of the code of your first UI test.
...
- **XCUIElement** is a proxy for UI elements in the application. Every `UIKit` class you can think of can be represented by an `XCUIElement` in the context of a UI test. How? `XCUIElement` can have a type (e.g. `.Cell`, `.Table`, `.WebView`, etc.) as well as an identifier. The identifier usually comes from the element's accessibility information, such as its accessibility identifier, label or value.
...
- **XCUIElementQuery** queries an `XCUIElement` for sub-elements matching some criteria. The three most popular ways to query elements is with `descendantsMatchingType(_:)`, `childrenMatchingType(:_)` and `containingType(_:)`.
...
You've got to check it out in action, so **Delete** the current contents of `testRaysFullBodyWorkout()`. Place your cursor inside the empty method, then click the **Record UI Test** button at the bottom of the editor:
...
> **Note**: There are a couple of things to keep in mind as you start writing UI tests: Although they _can_ be easier to write when they fail – especially if you're using UI recording – they can also be harder to debug than regular unit tests. They can also be quite brittle; if you make changes to your UI, you may need to dedicate some time to updating your tests. 
